...
export default function BehavioralIdentityProfileView() {
  ...
  return (
    <div className="h-full overflow-y-auto p-8 bg-[#F8FAFC] custom-scrollbar text-slate-800 pb-16">
      ...
      <div className="mb-2.5 flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
        ...
        <div className="mt-2 md:mt-0 flex items-center gap-1.5 bg-white px-2.5 py-1 border border-[#E2E8F0] rounded-lg">
          <Activity className="w-3.5 h-3.5 text-[#2563EB]" />
          <span className="text-[11px] font-mono text-slate-500 font-bold uppercase tracking-wide">
      ...
      <div className="grid grid-cols-4 gap-2.5 mb-6">
        ...
        <div className="bg-white border border-[#E2E8F0] px-3 py-2 rounded-xl shadow-sm">
          ...
          <div className="flex items-baseline gap-1 font-sans">
            <span className="text-[17px] font-black text-[#0f172a] tracking-tight leading-none">
              {selectedStudent.id === 'USR_89921_ROHAN' ? liveSessionStats.trustScore.toFixed(0) : selectedStudent.trustScore.toFixed(0)}%
            </span>
            <span className={`text-[11px] font-extrabold ${selectedStudent.trustScore >= 70 ? 'text-emerald-600' : 'text-amber-600'}`}>
              {selectedStudent.trustScore >= 70 ? '✓ COMPLIANT' : '⚡ EXCEPTION'}
            </span>
          </div>
          <p className="text-[11px] text-slate-450 mt-0.5 font-medium">Session trust rating</p>
        </div>
        ...
        <div className="bg-white border border-[#E2E8F0] px-3 py-2 rounded-xl shadow-sm">
          ...
          <div className="text-[17px] font-black text-[#2563EB] tracking-tight font-sans leading-none">
            {selectedStudent.identityMatch.toFixed(1)}%
          </div>
          <p className="text-[11px] text-slate-450 mt-0.5 font-medium">Biometric profile match</p>
        </div>

        {/* Metric 3: Risk Level */}
        <div className="bg-white border border-[#E2E8F0] px-3 py-2 rounded-xl shadow-sm">
          <span className="text-[11px] font-mono font-black uppercase text-slate-400 tracking-wider block mb-1">Risk Level</span>
          <span className={`px-2 py-0.5 rounded text-[11px] font-black uppercase border font-sans inline-block mt-0.5 ${
          ...
          <p className="text-[11px] text-slate-450 mt-0.5 font-medium">Bayesian classification</p>
        </div>
        ...
        <div className="bg-white border border-[#E2E8F0] px-3 py-2 rounded-xl shadow-sm">
          ...
          <div className="text-[11px] font-bold text-slate-800 tracking-tight line-clamp-1 leading-tight mt-0.5 uppercase">
            {selectedStudent.verdict === 'APPROVED' ? 'MAINTAIN PASSIVE VERIFICATION' : selectedStudent.verdict === 'REVIEW' ? 'CONDUCT MANUAL CADENCE REVIEW' : 'TERMINATE WORKSTATION SESSION'}
          </div>
          <p className="text-[11px] text-slate-450 mt-0.5 font-medium">Real-time prescription</p>
        </div>
      </div>
      ...
      {/* Live Session Card (light-themed) */}
      <div className="bg-white border border-[#E2E8F0] text-slate-800 p-4 rounded-2xl shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 relative overflow-hidden">
        {/* Background glow */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-blue-50/40 rounded-full blur-3xl pointer-events-none"></div>
        <div className="absolute bottom-0 left-0 w-48 h-48 bg-emerald-50/20 rounded-full blur-2xl pointer-events-none"></div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <strong className="text-sm font-sans tracking-tight text-slate-800">{selectedStudent.name}</strong>
              <span className="text-slate-350 font-mono text-xs">|</span>
              <span className="text-[11px] font-mono text-[#2563EB] bg-[#EFF6FF] px-2 py-0.5 rounded border border-[#93C5FD]">{selectedStudent.exam}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 md:gap-8 font-mono text-xs">
          <div>
            <span className="text-[11px] text-slate-400 uppercase block font-bold">Connected Time</span>
            <span className="text-[13px] font-bold text-slate-800 block mt-0.5">
              {Math.floor(liveSessionStats.connectedSeconds / 60)}m {liveSessionStats.connectedSeconds % 60}s
            </span>
          </div>
          <div>
            <span className="text-[11px] text-slate-400 uppercase block font-bold">Telemetry Captured</span>
            <span className="text-[13px] font-bold text-slate-800 block mt-0.5">
              {liveSessionStats.eventsProcessed.toLocaleString()} events
            </span>
          </div>
          <div>
            <span className="text-[11px] text-slate-400 uppercase block font-bold">Trust Score</span>
            <span className={`text-[13px] font-bold block mt-0.5 ${selectedStudent.trustScore > 70 ? 'text-emerald-600' : selectedStudent.trustScore > 40 ? 'text-amber-600' : 'text-red-600'}`}>
              {selectedStudent.id === 'USR_89921_ROHAN' ? liveSessionStats.trustScore.toFixed(0) : selectedStudent.trustScore.toFixed(0)}% TST
            </span>
          </div>
          <div>
            <span className="text-[11px] text-slate-400 uppercase block font-bold">Status</span>
            <span className={`inline-flex items-center gap-1 mt-0.5 font-bold uppercase tracking-wider text-[11px] ${
              selectedStudent.verdict === 'APPROVED' ? 'text-emerald-600' : selectedStudent.verdict === 'REVIEW' ? 'text-amber-600' : 'text-red-600'
            }`}>
              <span className={`w-1.5 h-1.5 rounded-full inline-block animate-pulse ${
                selectedStudent.verdict === 'APPROVED' ? 'bg-emerald-500' : selectedStudent.verdict === 'REVIEW' ? 'bg-amber-500' : 'bg-red-500'
              }`}></span>
        ...
        <div className="xl:col-span-8 space-y-6">
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-[0_1px_3px_rgba(0,0,0,0.05)] space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-[#F1F5F9]">
              ...
              <span className={`px-2.5 py-0.5 rounded text-[11px] font-sans font-extrabold uppercase border ${
                calibrationDone
                  ? 'text-emerald-700 bg-emerald-50 border-emerald-200'
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 text-xs font-mono">
              <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200/60 shadow-sm">
                <span className="text-[11px] text-slate-400 block uppercase font-bold">Typing Samples</span>
                <span className="text-[12.5px] font-bold text-[#0F172A] block mt-0.5">{typingCount} / 500</span>
              </div>
              <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200/60 shadow-sm">
                <span className="text-[11px] text-slate-400 block uppercase font-bold">Mouse Events</span>
                <span className="text-[12.5px] font-bold text-[#0F172A] block mt-0.5">{mouseCount.toLocaleString()} / 10,000</span>
              </div>
              <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200/60 shadow-sm">
                <span className="text-[11px] text-slate-400 block uppercase font-bold">Reading Actions</span>
              ...
              <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200/60 shadow-sm flex flex-col justify-center">
                <span className="text-[11px] text-slate-400 block uppercase font-bold">Baseline Quality</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="bg-[#F8FAFC] rounded-2xl p-4 border border-[#E2E8F0] space-y-3 shadow-inner">
                <div className="flex justify-between items-start">
                  <span className="text-[11px] font-mono font-bold text-slate-500 uppercase tracking-wider block">Identity Profile Match</span>
              ...
              <div className="bg-[#F8FAFC] rounded-2xl p-4 border border-[#E2E8F0] space-y-3 shadow-inner">
                <div className="flex justify-between items-start">
                  <span className="text-[11px] font-mono font-bold text-slate-500 uppercase tracking-wider block">Historical Consistency</span>
              ...
              <div className="bg-[#F8FAFC] rounded-2xl p-4 border border-[#E2E8F0] space-y-3 shadow-inner">
                <div className="flex justify-between items-start">
                  <span className="text-[11px] font-mono font-bold text-slate-500 uppercase tracking-wider block">Risk Threat Prominence</span>
              ...
              {selectedStudent.similarBehaviorSessions.map((session, sidx) => (
                <div key={sidx} className="bg-slate-50 p-3 rounded-xl border border-[#E2E8F0] flex justify-between items-center shadow-sm">
                  <div className="space-y-0.5">
                    ...
                    <span className="text-[11px] text-slate-500 block leading-tight">{session.description}</span>
                  </div>
                  <div className="text-right shrink-0">
                    <span className="text-[11px] font-mono text-slate-400 block uppercase font-bold">BEHAVIOR SIMILARITY</span>
        ...
        <div className="xl:col-span-4 space-y-6">
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-[0_1px_3px_rgba(0,0,0,0.05)] space-y-4">
            <div className="flex justify-between items-center border-b border-[#F1F5F9] pb-2.5">
              ...
              <span className="flex items-center gap-1 bg-red-50 text-red-700 border border-red-150 px-2 py-0.5 rounded text-[11px] font-mono font-bold">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="bg-slate-50 p-2.5 rounded-xl border border-[#E2E8F0]">
                <span className="text-[11px] font-mono text-slate-400 uppercase block">Keyboard Events</span>
              ...
              <div className="bg-slate-50 p-2.5 rounded-xl border border-[#E2E8F0]">
                <span className="text-[11px] font-mono text-slate-400 uppercase block">Mouse Events</span>
                <span className="text-sm font-bold text-[#0F172A] font-mono block mt-0.5">
                  {evidenceCounts[selectedStudent.id]?.mouseEvents.toLocaleString() ?? 0}
                </span>
              </div>
              <div className="bg-slate-50 p-2.5 rounded-xl border border-[#E2E8F0]">
                <span className="text-[11px] font-mono text-slate-400 uppercase block">Focus Events</span>
                <span className="text-sm font-bold text-[#0F172A] font-mono block mt-0.5">
                  {evidenceCounts[selectedStudent.id]?.focusChanges.toLocaleString() ?? 0}
                </span>
              </div>
              <div className="bg-slate-50 p-2.5 rounded-xl border border-[#E2E8F0]">
                <span className="text-[11px] font-mono text-slate-400 uppercase block">Tab Switches</span>
                <span className="text-sm font-bold text-[#0F172A] font-mono block mt-0.5">
                  {evidenceCounts[selectedStudent.id]?.tabSwitches.toLocaleString() ?? 0}
                </span>
              </div>
              <div className="bg-slate-50 p-2.5 rounded-xl border border-[#E2E8F0]">
                <span className="text-[11px] font-mono text-slate-400 uppercase block">Copy/Paste Attempts</span>
                <span className="text-sm font-bold text-[#0F172A] font-mono block mt-0.5">
                  {evidenceCounts[selectedStudent.id]?.copyAttempts.toLocaleString() ?? 0}
                </span>
              </div>
              <div className="bg-slate-50 p-2.5 rounded-xl border border-[#E2E8F0]">
                <span className="text-[11px] font-mono text-slate-400 uppercase block">Question Interactions</span>
                <span className="text-sm font-bold text-[#0F172A] font-mono block mt-0.5">
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-[0_1px_3px_rgba(0,0,0,0.05)] space-y-4 text-slate-800">
            <div className="flex justify-between items-center border-b border-[#F1F5F9] pb-2.5">
            ...
            <div className="space-y-3">
              {[
                ...
                { name: "Live Trust Signal", color: "bg-emerald-500", scale: selectedStudent.trustScore }
              ].map((sig, sIdx) => (
            ...
            {selectedProfileTab === 'details' ? (
              <div className="space-y-4 text-xs font-sans">
                <div className="space-y-1">
                  <span className="text-[11px] font-mono text-slate-400 uppercase font-black tracking-wider block">Session Workstation ID</span>
                ...
                {selectedStudent.verdict !== 'APPROVED' && (
                  <div className="bg-red-50/60 border border-red-200 rounded-xl p-3 space-y-2 text-[11px]">
                    ...
                    <div className="space-y-1 font-mono text-[11px]">
                      <div className="flex justify-between items-center bg-white px-2 py-0.5 rounded border border-red-100">
                        <span className="text-slate-500 font-sans">Typing Drift:</span>
                        <span className="font-bold text-red-600">
                          {selectedStudent.id === 'USR_89921_ROHAN' ? "+31%" : selectedStudent.id === 'USR_44112_NEHA' ? "+15%" : "+11%"}
                        </span>
                      </div>
                      <div className="flex justify-between items-center bg-white px-2 py-0.5 rounded border border-red-100">
                        <span className="text-slate-500 font-sans">Navigation Drift:</span>
                        <span className="font-bold text-red-600">
                          {selectedStudent.id === 'USR_89921_ROHAN' ? "+14%" : selectedStudent.id === 'USR_44112_NEHA' ? "+18%" : "+42%"}
                        </span>
                      </div>
                      <div className="flex justify-between items-center bg-white px-2 py-0.5 rounded border border-red-100">
                        <span className="text-slate-500 font-sans">Clipboard:</span>
                        <span className="font-bold text-red-600">
                          {selectedStudent.id === 'USR_89921_ROHAN' ? "+6" : selectedStudent.id === 'USR_44112_NEHA' ? "+2" : "+3"}
                        </span>
                      </div>
                      <div className="flex justify-between items-center bg-white px-2 py-0.5 rounded border border-red-100">
                        <span className="text-slate-500 font-sans">Identity Conf:</span>
                        <span className="font-bold text-[#2563EB]">
                          {selectedStudent.id === 'USR_89921_ROHAN' ? "44%" : selectedStudent.id === 'USR_44112_NEHA' ? "84%" : "89%"}
                        </span>
                      </div>
                      <div className="flex justify-between items-center bg-white px-2 py-0.5 rounded border border-red-100">
                        <span className="text-slate-500 font-sans">Action:</span>
                        <span className="font-bold text-red-655 text-red-600 font-sans font-bold text-[11px] uppercase truncate">
                ...
                <div className="bg-blue-50 border border-blue-105 p-3 rounded-xl space-y-1">
                  <span className="text-[11px] font-mono font-bold text-[#1E3A8A] uppercase tracking-wide flex items-center gap-1">
                ...
                <div className="border-l-2 border-slate-100 pl-4 space-y-4">
                  {selectedStudent.driftTimeline.map((item, idx) => (
                    <div key={idx} className="relative space-y-0.5 text-xs">
                      ...
                      <p className="text-[11px] text-slate-500">Telemetry variance: <strong className="font-mono text-slate-700">{item.drift}</strong></p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-[0_1px_3px_rgba(0,0,0,0.05)] space-y-3 select-none text-left">
            ...
            <p className="text-[11px] text-slate-500 leading-normal">
              Behavioral profiles mapped on our secure edge processing servers. Dimensions are processed with hardware verified verification algorithms.
            </p>
              ...
              {[
                ...
                return (
                  <div key={idx} className="relative flex items-start gap-3 text-left">
                    <span className={`absolute -left-[23px] top-0.5 w-5 h-5 rounded-full flex items-center justify-center font-mono text-[11px] font-bold border shadow-sm transition-all ${dotColor}`}>
                      {idx + 1}
                    </span>
                    <div className="flex-grow">
                      ...
                      <p className={`text-[11px] ${isActive ? 'text-[#64748B]' : 'text-slate-350'} mt-0.5`}>{step.desc}</p>
                    </div>
                  </div>
                );
              })}
